...
type FlatOpt = { grupo: GrupoCarroceria; item: string }

/**
 * Campo de carroceria com sugestões agrupadas: Abertas / Fechadas / Especiais.
 */
export function CarroceriaSuggestInput({
  value,
  onChange,
  ...
  const flat = useMemo(() => {
    ...
    const exact = GRUPOS_CARROCERIA.some((g) =>
      CARROCERIAS_POR_GRUPO[g].some((t) => t.toLowerCase() === q),
    )
    const out: FlatOpt[] = []
    for (const grupo of GRUPOS_CARROCERIA) {
      for (const item of CARROCERIAS_POR_GRUPO[grupo]) {
        if (!q || exact || item.toLowerCase().includes(q)) {
          out.push({ grupo, item })
        }
      }
    }
    // Sem filtro útil → mostra catálogo inteiro
    if (out.length === 0) {
      for (const grupo of GRUPOS_CARROCERIA) {
        for (const item of CARROCERIAS_POR_GRUPO[grupo]) {
          out.push({ grupo, item })
  ...
  const grouped = useMemo(() => {
    const map = new Map<GrupoCarroceria, string[]>()
    for (const { grupo, item } of flat) {
      const list = map.get(grupo) ?? []
      list.push(item)
      map.set(grupo, list)
    }
    return GRUPOS_CARROCERIA.map((g) => ({ grupo: g, items: map.get(g) ?? [] })).filter(
      (g) => g.items.length > 0,
    )
  }, [flat])
